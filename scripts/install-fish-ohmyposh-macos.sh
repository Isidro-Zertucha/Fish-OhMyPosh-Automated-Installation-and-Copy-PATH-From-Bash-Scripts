#!/usr/bin/env bash
# Fish and Oh My Posh Installation for macOS
# This script installs Fish shell, Oh My Posh, Nerd Fonts, and configures them on macOS

set -euo pipefail

# --- Configuration ---
NERD_FONT_URL="https://github.com/ryanoasis/nerd-fonts/releases/latest/download/FiraCode.zip"
OMP_BINARY_URL="https://github.com/JanDeDobbeleer/oh-my-posh/releases/latest/download/posh-darwin-amd64"
...
set_default_fish() {
  ...
  info "Changing default shell to fish for user $USER (this will require sudo)"
  if sudo chsh -s "$FISH_PATH" "$USER"; then
    success "Default shell changed to fish. Log out and back in for it to take effect."
  else
    warn "chsh failed. You may need to run: sudo chsh -s $FISH_PATH"
  fi
}
...
install_fonts() {
  ...
  if command -v wget >/dev/null 2>&1; then
    ...
    err "Neither wget nor curl available to download fonts."
    return
  fi
